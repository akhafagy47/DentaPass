...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#fff;border-radius:16px;overflow:hidden;">
  ...
  <tr><td style="padding:32px 32px 24px;">
    ...
    ${clinic.google_review_url ? `<a href="${clinic.google_review_url}" style="display:inline-block;margin-top:20px;padding:12px 28px;background:${accent};color:#fff;font-size:15px;font-weight:700;text-decoration:none;border-radius:10px;">Leave a review</a>` : ''}
  </td></tr>
...
Deno.serve(async () => {
  ...
  const { data: patients, error } = await supabase
    ...
    .select(`
      ...
      clinic:clinics(name, brand_color, google_review_url, owner_email)
    `)
